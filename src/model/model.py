...
class Model(torch.nn.Module):
    ...
    def forward(
        ...
        if self.inputs_dict is not None:
            temp = {}
            temp['inputs'] = batch
            if not self.is_encoding_mode:
                if 'labels' in self.inputs_dict.keys():
                    temp['labels'] = self.inputs_dict['labels']
            temp['attention_mask'] = self.inputs_dict['attention_mask']
            temp['t_rs'] = self.inputs_dict['t_rs']
            batch = temp
            print('Heatmap Case!')
        if prep_batch:
...
class EmbedderModel(torch.nn.Module):
    def __init__(self, original_model):
        ...
        self.embedder = self.original_model.embedder
    
    def forward (self, x):
        x = self.original_model.prep_batch(batch=x)
        x['inputs_embeds'] = self.embedder(batch=x)
        return x
...
class CustomModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return output['decoding_logits']
    
class EncoderModel(torch.nn.Module):
    def __init__(self, original_model):
        super(EncoderModel, self).__init__()
        self.original_model = original_model
        self.original_model.is_encoding_mode = True
