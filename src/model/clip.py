...
class Clipper(torch.nn.Module):
    ...
    def embed_text(self, text_samples, tokenized_text=None):
        if text_samples is not None and tokenized_text is None:
            clip_text = clip.tokenize(text_samples, context_length=77, truncate=True).to(self.device)
        elif tokenized_text is not None:
            clip_text = tokenized_text
        clip_text = clip_text.to(self.device)
