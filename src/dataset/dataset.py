...
class NsdDataset(Dataset):
    def __init__(self, data_dir, subject_id, mode='train', transform=None):
        ...
        # get absolute path
        self.nsd_files = [os.path.join(data_dir, subject_id, mode, f) for f in os.listdir(os.path.join(data_dir, subject_id, mode)) if f.endswith('.npy')]
    
    def __len__(self):
        return len(self.nsd_files)
    
    def __getitem__(self, index):
        # load npy file
        npy_file = self.nsd_files[index]
        data = np.load(npy_file, allow_pickle=True).item()
        if self.mode == 'train':
            # random select an index from 0 to 2
            index = np.random.randint(0, 3)
            fmri = data['fmri'][index]
        elif self.mode in ['val', 'test']:
            # take the mean of the fmri
            fmri = np.mean(data['fmri'], axis=0)
        # join the list of label to a string
        label = ' '.join([str(i) for i in data['label']])
        image = data['image']
        image = image / 255.0
...
def create_BOLD5000_dataset(path='../data/BOLD5000', patch_size=16, fmri_transform=identity,
            ...
            region_path='/scratch/yl6624/Data/BOLD5000_Processed'):
    ...
    fmri_inputs_train_major = []
    fmri_inputs_test_major = []
    fmri_trs_train_major = []
    fmri_trs_test_major = []
    fmri_attention_mask_train_major = []
    fmri_attention_mask_test_major = []
    for sub in subjects:
        # load fmri
        fmri_data_sub = []
        for roi in roi_list:
        ...
        test_fmri_inputs = []
        test_fmri_trs = []
        test_fmri_attention_mask = []
        for idx in test_idx:
            test_fmri_inputs.append(np.load(os.path.join(region_path, sub, 'test', f'{img_files[idx[0]]}_inputs.npy')))
            test_fmri_trs.append(np.load(os.path.join(region_path, sub, 'test', f'{img_files[idx[0]]}_t_rs.npy')))
            test_fmri_attention_mask.append(np.load(os.path.join(region_path, sub, 'test', f'{img_files[idx[0]]}_attention_mask.npy')))
            # test_image_name = str(img_files[idx[0]])
            # test_voxel = fmri_data_sub[idx[0]]
            # print("test image name",test_image_name)
            # print("test voxel shape",test_voxel.shape)
            # os.makedirs(os.path.join('/vast/yl6624/bold5000_voxel',str(sub),'test'), exist_ok=True)
            # np.save(os.path.join('/vast/yl6624/bold5000_voxel',str(sub),'test',f'{test_image_name}.npy'),test_voxel)
        test_fmri_inputs = np.concatenate(test_fmri_inputs, axis=0)
        test_fmri_trs = np.concatenate(test_fmri_trs, axis=0)
        test_fmri_attention_mask = np.concatenate(test_fmri_attention_mask, axis=0)
        ...
        train_fmri_inputs = []
        train_fmri_trs = []
        train_fmri_attention_mask = []
        for idx in train_idx:
            train_fmri_inputs.append(np.load(os.path.join(region_path, sub, 'train', f'{img_files[idx]}_inputs.npy')))
            train_fmri_trs.append(np.load(os.path.join(region_path, sub, 'train', f'{img_files[idx]}_t_rs.npy')))
            train_fmri_attention_mask.append(np.load(os.path.join(region_path, sub, 'train', f'{img_files[idx]}_attention_mask.npy')))
        #     train_image_name = str(img_files[idx])
        #     train_voxel = fmri_data_sub[idx]
        #     print("train image name",train_image_name)
        #     print("train voxel shape",train_voxel.shape)
        #     os.makedirs(os.path.join('/vast/yl6624/bold5000_voxel',str(sub),'train'), exist_ok=True)
        #     np.save(os.path.join('/vast/yl6624/bold5000_voxel',str(sub),'train',f'{train_image_name}.npy'),train_voxel)
        # exit()
        train_fmri_inputs = np.concatenate(train_fmri_inputs, axis=0)
        train_fmri_trs = np.concatenate(train_fmri_trs, axis=0)
        train_fmri_attention_mask = np.concatenate(train_fmri_attention_mask, axis=0)
        ...
        fmri_inputs_train_major.append(train_fmri_inputs)
        fmri_inputs_test_major.append(test_fmri_inputs)
        fmri_trs_train_major.append(train_fmri_trs)
        fmri_trs_test_major.append(test_fmri_trs)
        fmri_attention_mask_train_major.append(train_fmri_attention_mask)
        fmri_attention_mask_test_major.append(test_fmri_attention_mask)
    ...
    # os.makedirs(os.path.join('avg_test_origin'), exist_ok=True)
    # print(len(test_idx))
    # for idx, img in enumerate(img_test_major):
    #     img_name = img_files[test_idx[idx][0]]
    #     print(img_name)
    #     print("img",img)
    #     print(type(img))
    ...
    #     img.save(os.path.join('avg_test_origin','{}_{}.png'.format(img_name,idx)))
    # exit()

    fmri_inputs_train_major = np.concatenate(fmri_inputs_train_major, axis=0)
    fmri_inputs_test_major = np.concatenate(fmri_inputs_test_major, axis=0)
    fmri_trs_train_major = np.concatenate(fmri_trs_train_major, axis=0)
    fmri_trs_test_major = np.concatenate(fmri_trs_test_major, axis=0)
    fmri_attention_mask_train_major = np.concatenate(fmri_attention_mask_train_major, axis=0)
    fmri_attention_mask_test_major = np.concatenate(fmri_attention_mask_test_major, axis=0)
    ...
    if isinstance(image_transform, list):
        return (BOLD5000_dataset(fmri_train_major, img_train_major, fmri_transform, image_transform[0], num_voxels, 
                                 fmri_inputs_train_major, fmri_trs_train_major, fmri_attention_mask_train_major), 
                BOLD5000_dataset(fmri_test_major, img_test_major, torch.FloatTensor, image_transform[1], num_voxels,
                                 fmri_inputs_test_major, fmri_trs_test_major, fmri_attention_mask_test_major))
    else:
...
class BOLD5000_dataset(Dataset):
    def __init__(self, fmri, image, fmri_transform=identity, image_transform=identity, num_voxels=0,
                 fmri_inputs=None, fmri_trs=None, fmri_attention_mask=None):
        self.fmri = fmri
        self.image = image
        self.fmri_transform = fmri_transform
        self.image_transform = image_transform
        ...
        self.fmri_inputs = fmri_inputs
        self.fmri_trs = fmri_trs
        self.fmri_attention_mask = fmri_attention_mask
    ...
    def __getitem__(self, index):
        ...
        fmri_inputs = self.fmri_inputs[index]
        # fmri_inputs = np.expand_dims(fmri_inputs, axis=0)
        fmri_trs = self.fmri_trs[index]
        # fmri_trs = np.expand_dims(fmri_trs, axis=0)
        fmri_attention_mask = self.fmri_attention_mask[index]
        # fmri_attention_mask = np.expand_dims(fmri_attention_mask, axis=0)
        return {'fmri': self.fmri_transform(fmri), 'image': self.image_transform(img),
                'inputs': fmri_inputs, 't_rs': fmri_trs, 'attention_mask': fmri_attention_mask}
