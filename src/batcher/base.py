...
class BaseBatcher:
    ...
    def preprocess_sample(
        ...
        if 'config.json' in out and 'stimuli.png' in out:
            img_label = str(out['config.json']['label'])
            img = out['stimuli.png']
            img_name = str(out['config.json']['img_name'])
            if self.pad_voxel:
                voxels = out['voxel.pyd']
                voxels = np.expand_dims(voxels, axis=0)
                voxels = _pad_to_patch_size(voxels, 2787)
                voxels = torch.from_numpy(voxels).to(torch.float)
                voxels = voxels.squeeze(0)

                assert len(voxels) == 2787, f"Voxel shape is {len(voxels)}, not equal to 2787"
            else:
                voxels = torch.from_numpy(out['voxel.pyd']).to(torch.float)
            out['voxels'] = voxels
        elif 'config.json' in out:
            scene_class = out['config.json']['class']
            text = str(out['config.json']['text'])
            scenario = str(out['config.json']['scenario'])
            subject = int(out['config.json']['sub'][-2:]) - 1
        if self.sample_keys is not None:
            out = {
                key: out[key] 
                for key in self.sample_keys
        ...
        if img_label and img:
            out['img_label'] = img_label
